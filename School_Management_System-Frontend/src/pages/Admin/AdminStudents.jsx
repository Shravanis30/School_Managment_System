    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/students/register`,
        {
          ...
          rollNo: registerForm.enrollmentNo,
      ...
      setRegisterError(err.response?.data?.message || "Student registration failed. Please try again.");
    }
  };
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white">
              ...
              <tbody>
                {students.map((s, i) => (
                  <tr key={s._id} className="bg-white/5 hover:bg-white/10 border-b border-white/10">
                    <td className="p-2">{i + 1}</td>
                    <td className="p-2">{s.fullName || s.name}</td>
                    <td className="p-2">{s.rollNumber || s.rollNo || '-'}</td>
                    <td className="p-2">{s.className || '-'}</td>
                    <td className="p-2">{s.section || '-'}</td>
                    <td className="p-2">
                      <button
      ...
      {showRegisterModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-xl w-full max-w-md border border-white/10">
            <div className="p-6">
