...
const TeacherAssignments = () => {
  ...
  useEffect(() => {
    if (form.classId) {
      // Get subjects from the classes list we already fetched
  ...
  useEffect(() => {
    if (filter.classId) {
      // Get subjects from the classes list we already fetched
      const selectedClass = classes.find(cls => cls._id === filter.classId);
      if (selectedClass) {
        setSubjects(selectedClass.subjects || []);
      }
    }
  }, [filter.classId, classes]);
  ...
  const fetchSubmissions = async () => {
    if (filter.className && filter.subject) {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/assignments/submissions/${filter.className}/${filter.subject}`,
          { credentials: 'include' }
        );
        const data = await res.json();
        setSubmissions(data || []);
