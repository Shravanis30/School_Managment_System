...
const StudentAssignments = () => {
  ...
  const [submitting, setSubmitting] = useState(false);
  const [success, setSuccess] = useState(false);
  const [studentInfo, setStudentInfo] = useState(null); // Added to store student info
  ...
  useEffect(() => {
    const fetchStudentAndAssignments = async () => {
      try {
        ...
        const student = await studentRes.json();
        setStudentInfo(student); // Store student info
  ...
  const handleSubmit = async (assignmentId) => {
    if (!file) return alert('Please select a file to upload');
    if (!studentInfo || !studentInfo.adminId) { // Added validation
      alert('Student information is incomplete. Please refresh the page.');
      return;
    }
    ...
    const formData = new FormData();
    formData.append('assignmentId', assignmentId);
    formData.append('adminId', studentInfo.adminId); // Added adminId
    formData.append('file', file);
