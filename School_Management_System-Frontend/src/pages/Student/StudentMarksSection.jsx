...
const StudentMarksSection = () => {
  ...
  const transformMarksData = (data) => {
    return data.flatMap(entry => 
      Object.entries(entry.marksPerSubject).map(([subject, marksValue]) => ({
        id: `${entry._id}-${subject}`,
        team: entry.team,
        subject,
        marks: marksValue,
        total: entry.totalMarksPerSubject,
        percentage: ((marksValue / entry.totalMarksPerSubject) * 100).toFixed(2),
        date: new Date(entry.createdAt).toLocaleDateString(),
      }))
    );
  };
          ...
          <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleDownloadPDF}
                disabled={generatingPDF || filteredData.length === 0}
                className={`bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-semibold px-4 py-2 rounded-lg shadow flex items-center ${
                  generatingPDF || filteredData.length === 0 ? "opacity-70 cursor-not-allowed" : ""
                }`}
              >
                {generatingPDF ? (
