...
// ✅ POST create meeting and send emails (Admin only)
router.post("/add", authMiddleware, authorizeRole("admin"), async (req, res) => {
  try {
    ...
    // Send email only to this admin's teachers
    const teachers = await Teacher.find({ adminId: req.user._id }, "email");
