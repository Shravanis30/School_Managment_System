...
export const setFeeStructure = async (req, res) => {
  ...
  let fs = await FeeStructure.findOne({ adminId, className });
  if (fs) {
    fs.term1 = term1; fs.term2 = term2;
    await fs.save();
  } else {
    fs = await FeeStructure.create({ adminId, className, term1, term2 });
  }
  res.json(fs);
};
...
export const getFeeStructure = async (req, res) => {
  ...
  const fs = await FeeStructure.findOne({ adminId, className });
  if (!fs) throw new ApiError(404, "Not found");
  res.json(fs);
};
...
export const payFee = async (req, res) => {
  ...
  res.json({ paidTerm1: updatedFee.paidTerm1, paidTerm2: updatedFee.paidTerm2 });
};
...
export const getCollection = async (req, res) => {
  try {
    ...
    const transactions = await PaymentTransaction.find({ studentId: { $in: studentIds } })
      .populate("studentId", "name className email rollNo")
      .sort({ createdAt: -1 });
...
export const getPending = async (req, res) => {
  try {
    const adminId = req.user._id;
    const students = await Student.find({ adminId }).select("name className");
    const feeStructures = await FeeStructure.find({ adminId });
